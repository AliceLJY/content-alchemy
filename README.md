...
由 3 个 Claude Code Agent 组成评审团队（架构师、安全审计、DX 体验），对整个仓库做了一轮代码审查 + 实测验证：

| 你可能遇到的问题 | 我已经帮你解决了 |
|----------------|-----------------|
| 🤔 "clone 下来跑不了，路径报错" | 移除所有硬编码路径，`publish.sh` 基于脚本位置自动检测 |
| 🤔 "两个 gemini 脚本不知道用哪个" | v1/v2 合并为单文件，`--method auto\|api\|cdp` 一个入口搞定 |
| 🤔 "装了依赖还是缺包" | 补上遗漏的 `juice`，新增项目元数据和 `scripts` 快捷命令 |
| 🤔 "配图时 Claude 不问我就直接生了" | 强化 nano-banana-pro Checkpoint，确保不跳过用户确认 |
| 🤔 "发布时让我手动启动 Chrome" | Chrome 由脚本自动处理，不再要求手动启动 |
| 🤔 "baoyu-danger-gemini-web 报 unknown skill" | 明确指向 `bun scripts/gemini-image-gen.ts` 脚本入口 |
...
| 🤔 "来源有了但数据本身是错的" | **Stage 3.5 交叉验证**：多源交叉印证 + 概念混淆检查 + 绝对化表述扫描 |
| 🤔 "AI 写的数字不敢信" | **置信度自评**：写作时实时评估每条数据，不确定就降级或删除 |
| 🤔 "去了 AI 味但整体还是像 AI" | **六维 AI 味扫描**：结构/句式/词汇/情感/元评论/可预测性全面检查 |
